class Parser
	...
	if @prog[0] !~ /[0-9]+/ then
	    raise "Syntax error: no line number: "+@prog[0..3].join(" ")
	end
	lineno = @prog.shift
        t = ParseTreeNode.new("Statement",[],nil)
	t.child.push(ParseTreeNode.new("Label",nil,"L"+lineno))
        if @prog[0] == "if" then
            @prog.shift
            e = parseExpression
            if @prog[0] != "goto" then
            ...
            @prog.shift
	    t.shild.push(ParseTreeNode.new("End",nil,"End"))   
	    return t
...
class QuadrupleGenerator
    ...
    def generate(n)
        ...
        when "Statement"
	    # first child should be the line number
